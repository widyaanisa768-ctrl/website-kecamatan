...
export default function SuratTanahForm() {
  ...
  const validate = () => {
    ...
    for (const key of requiredFiles) {
      ...
      const hasOld =
        (key === 'alasHak' && !!editing?.dokumen?.suratDasarAlasHak?.name) ||
        (key === 'ahliWaris' && !!editing?.dokumen?.suratKeteranganAhliWaris?.name) ||
        (key === 'ktp' && !!editing?.dokumen?.fotocopyKtp?.name) ||
        (key === 'blangko' && !!editing?.dokumen?.blangkoSktSkgrBermaterai?.name) ||
        (key === 'fotoLokasi' && !!editing?.dokumen?.fotoLokasiTanah?.name) ||
        (key === 'pbb' && !!editing?.dokumen?.buktiSetoranPbb?.name)
