...
export default function AktaKelahiranForm() {
  ...
  const validate = () => {
    ...
    for (const key of requiredFiles) {
      ...
      const hasOld =
        (key === 'suratLahir' && !!editing?.dokumen?.suratKeteranganLahir?.name) ||
        (key === 'ktpOrtu' && !!editing?.dokumen?.ktpKeduaOrangTua?.name) ||
        (key === 'kk' && !!editing?.dokumen?.kk?.name) ||
        (key === 'suratNikah' && !!editing?.dokumen?.suratNikah?.name) ||
        (key === 'surat_rekomendasi_lurah' && !!editing?.dokumen?.surat_rekomendasi_lurah?.name)
