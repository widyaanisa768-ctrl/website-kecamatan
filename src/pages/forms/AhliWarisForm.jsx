...
export default function AhliWarisForm() {
  ...
  const validate = () => {
    ...
    for (const key of requiredKeys) {
      ...
      const hasOld =
        ...
        (key === 'kk' && !!editing?.dokumen?.fotocopyKkAhliWaris?.name) ||
        (key === 'kematian' && !!editing?.dokumen?.suratKeteranganKematian?.name)
  ...
  const onSubmit = (e) => {
    ...
    try {
      const keteranganPemohon = `Pewaris: ${form.nama_pewaris} • Pemohon: ${form.nama_pemohon}`
      if (editing) {
        const nextStatus =
